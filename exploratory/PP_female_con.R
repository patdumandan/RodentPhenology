#FINAL MODEL FOR REPRODUCTIVE PHENOLOGY OF PP FEMALES IN CONTROL####
...
X1= PP_female_con$month
X2= PP_female_con$year
X3= PP_female_con$ndvis_lag
X4= PP_female_con$temps_lag_mean
X5= PP_female_con$ppts_lag_warm
X6= PP_female_con$ppts_lag_cool
X7= PP_female_con$ppts_cool
X8= PP_female_con$ppts_warm
X9= PP_female_con$temps_mean
X10= PP_female_con$ndvis
num_data = length(X1)
B1 = t(bs(X1, df=NULL, knots=NULL, degree=3, intercept = FALSE))
B2 = t(bs(X2, df=NULL, knots=NULL, degree=3, intercept = FALSE))
num_basis1 = nrow(B1)
num_basis2 = nrow(B2)
Y = PP_female_con$reproductive
n=PP_female_con$abundance

dat_list1=list(X1= PP_female_con$month,
               X2= PP_female_con$year,
               X3= PP_female_con$ndvis_lag,
               X4= PP_female_con$temps_lag_mean,
               X5= PP_female_con$ppts_lag_warm,
...
print(mod1, pars=c ("a0", "ndvi_eff", "warm_ppt_eff", "cool_ppt_eff", "temp_eff",
                    "ndvi_lag_eff", "warm_ppt_lag_eff", "cool_ppt_lag_eff", "temp_lag_eff"))

ppex_f1=mgcv::gam(proportion~s(month, bs="cc")+s(year)+ndvis+temps_mean+ppts_warm+ppts_cool+
...
dat_list2=list(X1= PP_female_con$month,
               X2= PP_female_con$year,
               X3= PP_female_con$ndvis_lag,
               X4= PP_female_con$temps_lag_mean,
               X5= PP_female_con$ppts_lag_warm,
...
dat_list3=list(X1= PP_female_con$month,
               X2= PP_female_con$year,
               X3= PP_female_con$ndvis_lag,
               X4= PP_female_con$temps_lag_mean,
               X5= PP_female_con$ppts_lag_warm,
               ...
               X10= PP_female_con$ndvis,
               X11= PP_female_con$pps,
               X12= PP_female_con$pbs,
               X13= PP_female_con$dipos,
               num_data = length(X1),
               B1 = t(bs(X1, df=NULL, knots=NULL, degree=3, intercept = FALSE)), 
               B2 = t(bs(X2, df=NULL, knots=NULL, degree=3, intercept = FALSE)),
               num_basis1 = nrow(B1),
...
ppex_f3=mgcv::gam(proportion~s(month, bs="cc")+s(year)+ndvis+temps_mean+ppts_warm+ppts_cool+
                    ndvis_lag+ temps_lag_mean+ppts_lag_warm+ppts_lag_cool+pps+pbs+dipos,
                  data=PP_female_con, weights=abundance, family=binomial(link="logit"))
summary(ppex_f3)
...
post2=t(post2)
t2=cbind(PP_female_con$month, post2)
t2=as.data.frame(t2)
t2=t2%>%
  rename("month"="V1")
...
axis(1, c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sept", "Oct", "Nov", "Dec"), at=c(1:12))
points(PP_female_con$proportion~PP_female_con$month, col="blue", pch=16)
lines(smooth.spline(X1, Y_hat_med), col="red")

#looic calculations####
loglik1=extract_log_lik(mod1)
loo(loglik1)
...
q=extract(mod3)
q=as.data.frame(q)
d1=mcmc_areas(q, pars=c ("ndvi_eff", "warm_ppt_eff", "cool_ppt_eff", "temp_eff",
                         "ndvi_lag_eff", "warm_ppt_lag_eff", "cool_ppt_lag_eff", "temp_lag_eff",
                         "pp_eff", "pb_eff", "dipo_eff"),prob=.80, prob_outer=0.95,
              point_est = "median")+ggtitle("PP female (exclosure)")
d1
