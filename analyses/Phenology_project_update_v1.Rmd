...
Reproductive phenology is a key descriptor of an important ecological strategy of organisms (Schwartz, 2003). It is a period that requires high energetic costs for individuals which is why efforts need to be optimized such that they are able to exploit resources while avoiding unfavorable conditions (Bradshaw and Holzapfel 2007). Thus, identifying the abiotic (e.g., climate change) and biotic conditions (e.g., presence of competitors) that may drive shifts in the timing of these reproductive events is important in understanding how the demography of wildlife populations may vary in future landscape scenarios. However, our understanding of how most organisms time these events given their environmental context is generally lacking. This is especially true for nocturnal small mammals that are often challenging to obtain data for. Here, I attempt to construct the reproductive phenology of rodents monitored in a study site near Portal, Arizona. I also describe the phenophases of rodents at different biotic conditions to test inferences on the effects of competititve interactions on small mammal reproduction.  
...
*Model Variations*  
*Note: I am currently exploring two models to address my research question: 1) hierarchical beta-binomial model with trigonometric functions (sine and cosine terms to account for periodicity) and 2) hierarchical BB model with an autocorrelation term (to account for non-independence in monthly data)*     
...
*Hierarchical BB with cosinor terms*
...
### **WOULD LOVE TO GET FEEDBACK/YOUR THOUGHTS ON**  
* cycle/period is currently "fixed" not allowed to vary per year. any idea how I could add that in the model?    
* temporal autocorrelation: how do people usually address this?  
* should I build a "composite" (all species so keep being hierarchical in that sense) or model each species separately?  
* how do you interpret the paraemters for the sine-cosine terms?   
...
### **WOULD LOVE TO GET FEEDBACK/THOUGHTS ON**  
* thoughts on other models that I could use where I can easily account for autocorrelation that doesn't take forever to run (this ran for 3 hours with just 300 iterations. maybe any thoughts on how to optimize it?  
